...
in_ch      = 4
resolution = 64
checkpoint = r"C:\Users\noahm\PlasmaDiffusionModel\3_31_25_full_train_small\plasma_full_latest.pt"         # <-- your .pt file
...
start_t  = 75      # starting time index — frames [10, 11, 12] condition → predict 13
...
with torch.no_grad():
    ...
    condition = condition,
    cfg_scale = 2.0,
    steps     = 50,      # try 50, 100, 200 and compare
...
# ── Shared color scale across predicted and ground truth ──────────────────
vmin = min(pred_density.min(), rho_gt.min())
vmax = max(pred_density.max(), rho_gt.max())
...
for ax, axis, label in zip(axes[0], [0, 1, 2], ["XY (z=mid)", "XZ (y=mid)", "YZ (x=mid)"]):
    ...
    im  = ax.imshow(slc, cmap="inferno", origin="lower", vmin=vmin, vmax=vmax)
