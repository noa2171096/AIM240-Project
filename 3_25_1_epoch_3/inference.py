...
in_ch      = 4
resolution = 64
checkpoint = "plasma_test_latest.pt"          # <-- your .pt file
data_file  = r"C:/mhd_data/data/valid/MHD_Ma_0.7_Ms_0.5.hdf5"  # <-- any HDF5 file

# ── 1. Rebuild model ─────────────────────────────────────────────────────────
base_model = ModelBase(
    ch=32, ch_mult=(1, 2, 4, 8), node_dim=256,
    edge_dim=64, in_channels=4, out_channels=4,
    resolution=64, k=16,num_mp_layers=8
).to(device)
...
start_t  = 10      # starting time index — frames [10, 11, 12] condition → predict 13
...
# ── 5. Visualize 2D — predicted vs ground truth ─────────────────────────────
mid = resolution // 2
...
plt.tight_layout()
plt.savefig("density_2d_comparison_1.png", dpi=150)
plt.show()

# ── 6. Visualize 3D — isosurface scatter plot ────────────────────────────────
fig = plt.figure(figsize=(14, 6))

for idx, (data, title) in enumerate([(pred_density, "Predicted"), (rho_gt, "Ground Truth")]):
    ax = fig.add_subplot(1, 2, idx + 1, projection="3d")

    threshold = np.percentile(data, 30)
    z, y, x = np.where(data > threshold)
    vals = data[data > threshold]

    sc = ax.scatter(x, y, z, c=vals, cmap="inferno", s=1, alpha=0.3)
    ax.set_title(f"{title} Density (top 20%)")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")
...
plt.tight_layout()
plt.savefig("density_3d_comparison_1.png", dpi=150)
plt.show()
